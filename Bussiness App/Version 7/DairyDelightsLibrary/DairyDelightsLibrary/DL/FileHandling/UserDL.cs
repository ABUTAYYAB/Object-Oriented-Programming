        ...
        private List<User> GetDataFromUserFile(string path)
        {
            ...
            if (File.Exists(path))
            {
                ...
                while ((record = fileVariable.ReadLine()) != null)
                {
                    ...
                    /*if (Validation.UserValidation.IsStringValid(userName) && Validation.UserValidation.IsStringValid(userPassword) && Validation.UserValidation.IsStringValid(userRole))
                    {
                        if (CheckIfUserNameAlreadyExist(userName))
                        {
                            if (userRole == "Admin" || userRole == "Worker" || userRole == "DeliveryBoy" || userRole == "Customer")
                            {
                                User user1 = new User(userName, userPassword, userRole);
                                UsersList.Add(user1);
                            }
                        }
                    }*/
