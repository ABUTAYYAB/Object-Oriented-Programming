...
namespace DairyDelightsLibrary.DL.DataBase
{
    public class ProductDL : IProduct
    {
        ...
        public bool ChangePriceByName(string name, string Price)
        {
            ...
            SqlConnection connection = new SqlConnection(connectiongString);
            connection.Open();
            string query = string.Format("UPDATE Credentials SET Price = '{0}' WHERE ProductName = '{1}'", Price, name);
            SqlCommand cmd = new SqlCommand(query, connection);
            int rows = cmd.ExecuteNonQuery();
            connection.Close();
            if (rows > 0)
        ...
        public bool ChangeDescriptionByName(string name, string Description)
        {
            ...
            SqlConnection connection = new SqlConnection(connectiongString);
            connection.Open();
            string query = string.Format("UPDATE Credentials SET Description = '{0}' WHERE ProductName = '{1}'", Description, name);
            SqlCommand cmd = new SqlCommand(query, connection);
            int rows = cmd.ExecuteNonQuery();
            connection.Close();
            if (rows > 0)
        ...
        public bool ChangeQuantityByName(string name, string Quantity)
        {
            ...
            SqlConnection connection = new SqlConnection(connectiongString);
            connection.Open();
            string query = string.Format("UPDATE Credentials SET Quantity = '{0}' WHERE ProductName = '{1}'", Quantity, name);
            SqlCommand cmd = new SqlCommand(query, connection);
            int rows = cmd.ExecuteNonQuery();
            connection.Close();
            if (rows > 0)
        ...
        public bool ChangeDiscountByName(string name, string discount)
        {
            ...
            SqlConnection connection = new SqlConnection(connectiongString);
            connection.Open();
            string query = string.Format("UPDATE Credentials SET Discount = '{0}' WHERE ProductName = '{1}'", discount, name);
            SqlCommand cmd = new SqlCommand(query, connection);
            int rows = cmd.ExecuteNonQuery();
            connection.Close();
            if (rows > 0)
        ...
        public List<Product> GetProductsList() 
        {
            ...
            while (reader.Read())
            {
                ...
                Product1.SetReviews(list);

                //string ReviewsInString = Product1.GetReviewsInString();
                //List<string> Reviews = ConvertStringIntoReviewsList(ReviewsInString);
